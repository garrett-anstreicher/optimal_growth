...
k_grid = collect(range(.01, length = 1000, stop = 45.0)) #capital grid
...
function Bellman(val_func, pol_func)
    ...
    for i_k = 1:nk, i_z = 1:nz #loop over state space
        candidate_max = -Inf #something crappy
...
#### 1569.719437 seconds (16.21 G allocations: 452.636 GiB, 3.99% gc time)
